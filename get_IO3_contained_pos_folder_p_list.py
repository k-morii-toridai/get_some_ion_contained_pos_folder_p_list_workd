...
from package_bond_search_algorithm.algolithm_bond_search_for_IO3 import concat_filter
...
I_O_existed_poscar_folder_path_list = np.load(target_npy_p, allow_pickle=True)
print(f"len(I_O_existed_poscar_folder_path_list): {len(I_O_existed_poscar_folder_path_list)}")

# ターゲットとなるイオンの元素種を含むPOSCAR.nnlistのディレクトリパス一覧を取得
add_str = '/nnlist_5/POSCAR.nnlist'
I_O_existed_poscar_nnlist_path_list = [str(p) + add_str for p in I_O_existed_poscar_folder_path_list]

# 並列化
pp = Pool(cpu_count() - 1)
total = len(I_O_existed_poscar_nnlist_path_list)
try:
    bool_ion_contain = list(tqdm(pp.imap(wrap_nnlist2df_and_concat_filter, I_O_existed_poscar_nnlist_path_list), total=total))
finally:
    pp.close()
    pp.join()

# apply filter(:bool_ion_contain) to I_O_existed_poscar_nnlist_path_list
ion_contained_poscar_nnlist_path_list = np.array(I_O_existed_poscar_nnlist_path_list)[bool_ion_contain]
...
saved_npy_fname = 'IO3_contained_poscar_folder_path_list.npy'
np.save(saved_npy_fname, ion_contained_poscar_folder_path_list)
# print parcentage
print(f"len(ion_contained_poscar_folder_path_list)/len(I_O_existed_poscar_folder_path_list) :\
